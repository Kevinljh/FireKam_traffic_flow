...
namespace MotionDetection
{
   public partial class FireKAM : MetroForm
   {
        ...
        //report heat map
        Heatmap reportHeatMap = new Heatmap();
      ...
      protected override void Dispose(bool disposing)
      {
        ...
        private void ToPDF_Click(object sender, EventArgs e)
        {
            ...
            {
                Document doc = new Document(PageSize.LETTER, 10, 10, 45, 35);
                PdfWriter wri = PdfWriter.GetInstance(doc, new FileStream(sfd.FileName, FileMode.Create));
                doc.Open();
                var chartimage = new MemoryStream();
                if (ReportTypeCB.SelectedIndex == 5)
                {
                    reportHeatmapImageBox.BackgroundImage.Save(chartimage, ImageFormat.Png);
                }
                else
                {
        ...
        private async void GenerateChartBtn_Click(object sender, EventArgs e)
        {
            ...
            if (ReportTypeCB.SelectedIndex == 5)
            {
                reportHeatMap.heatPoints.Clear();
                List<Point> results = await DataAccess.FindMotionPoints(GetDateTimeString(dateTimePicker1.Value), GetDateTimeString(dateTimePicker2.Value.AddDays(1).AddMilliseconds(-1)), ReportSourceDropdown.Text);
                if (results != null)
                {
                    foreach (Point result in results)
                    {
                        reportHeatMap.heatPoints.Add(new HeatPoint(result.X, result.Y, 32));
                    }
                }
                reportHeatmapImageBox.Visible = true;
                Pen p = new Pen(Color.Black, 2);
                System.Drawing.Image temp = reportHeatMap.CreateHeatmap();
                temp = ResizeImage(temp, reportHeatmapImageBox.Width+180, reportHeatmapImageBox.Height+180);
                using (Graphics g = Graphics.FromImage(temp))
                {
                    for (int i = 1; i < 7; i++)
                    {
                        // Vertical
                        g.DrawLine(p, i * reportHeatmapImageBox.Width/6, 0, i * reportHeatmapImageBox.Width / 6, reportHeatmapImageBox.Height);
                        // Horizontal
                        g.DrawLine(p, 0, i * reportHeatmapImageBox.Height/6, reportHeatmapImageBox.Width, i * reportHeatmapImageBox.Height/6);
                    }
                }
                
                reportHeatmapImageBox.BackgroundImage = temp;
            ...
            {
                ChartTypeSwitch("pie");
                reportHeatmapImageBox.Visible = false;
                for (int i = 1; i < 37; i++)
                {
                    List<BsonDocument> result = await DataAccess.FindByTimeAndArea(GetDateTimeString(min), GetDateTimeString(max.AddDays(1).AddMilliseconds(-1)), i, ReportSourceDropdown.Text);
                    if (result != null)
                    {
                ...
                foreach (DataPoint point in chart1.Series[0].Points)
                {
                    ...
                    {
                        total += (double)point.YValues.GetValue(0);
                    }
                }
                double filterPercentage = total * 0.01;
                double otherTotalValue = 0;
                foreach (DataPoint point in chart1.Series[0].Points)
                {
                    if (point.YValues.Length > 0)
                    {
                        if((double)point.YValues.GetValue(0) < filterPercentage)
                        {
                            otherTotalValue += (double)point.YValues.GetValue(0);
                        }
                    }
                }
                //ignore area less than 1%
                DataManipulator myDataManip = chart1.DataManipulator;               
                myDataManip.Filter(CompareMethod.LessThan, filterPercentage, chart1.Series[0]);
            }
            //yearly report
        ...
        private void ReportTypeCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (ReportTypeCB.SelectedIndex == 5)
            {
                label8.Visible = true;
                dateTimePicker2.Visible = true;
